...
在拼音上**點一下**就能在該處插入或取消一個空格,立刻反映在畫面上:

- 點某個字母的**左半邊** → 切在它前面;**右半邊** → 切在它後面
- 已經有空格的地方再點一次就取消(kuroshiro 自己斷的空格也可以取消)
...
| **雙擊拼音** | 這個詞**讀錯了**(轉得出來但是錯的,例如 心の臓 → kokoro no) | 修正讀音 | 空的,由使用者自己點 |

雙擊那條路刻意**不猜**預選範圍:轉換過程沒有留下「這幾個拼音字母來自原文哪幾個字」的對應關係,只能照比例估,而估錯的預選比沒有預選更糟 —— 使用者會以為那就是我們認定的詞,直接填讀音存下去,結果修到隔壁那個字。
