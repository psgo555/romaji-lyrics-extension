 ...
 *
 * ── 一次誤判的紀錄(2026-08)────────────────────────────────
 * 使用者回報「沒有同步歌詞的歌,連 Spotify 內建的跟唱置中也不見了」,
 * 我判定是這支造成的並整個拿掉 —— 理由是捲動事件分不出來源,
 * Spotify 有可能把我們的 scrollBy 當成使用者自己捲,因而停掉自動置中。
 *
 * **那個推論是錯的。** 關掉整個擴充功能之後症狀一模一樣,而且 Spotify
 * 自己在那首歌上方就寫著「這些歌詞尚未與歌曲同步」—— 那首歌根本沒有
 * 時間軸,它從來就不會跟。
 *
 * 留這段話是因為那個推論聽起來很合理,下次還會有人(包括我)想到同一件事。
 * 真的要懷疑這支的時候,**先關掉整個擴充功能比對**,那比讀程式碼快得多。
 *
 * 風險本身仍然存在(我們確實在動一個 Spotify 在管的容器),
 * 只是目前沒有任何證據顯示它發生過。
 *
 * ── 為什麼目標位置寫死在正中間 ──────────────────────────────
