...
export const CORRECTIONS = [
  ...
  /*
   * 「心の臓」是心臟的文言說法,固定讀 しんのぞう。
   *
   * kuromoji 在這裡犯了兩個疊在一起的錯:「心」被當成單獨的詞讀成 こころ,
   * 「臓」則完全讀不出來、原樣掉出來,結果是 kokoro no 臓。
   *
   * surface 一定要是**整個詞**。只補「臓 → ぞう」的話前半段還是錯的
   * (會變成 kokoro no zou)—— 這正是補讀音面板要求涵蓋整個詞的原因。
   *
   * 實測於 RADWIMPS〈すずめ〉:それらが結ばれるのは 心の臓
   */
  { surface: '心の臓', reading: 'しんのぞう' },

  /*
   * 「熄む」(やむ)是熄滅、止息,這裡收的是連用形「熄み」。
   *
   * 為什麼不寫成「熄 → や」:那個字在別的詞裡讀 そく(熄滅 = そくめつ),
   * 單字替換會把那些詞一起弄壞。整個詞收進來才安全 ——
   * 這張表的規則一向是長詞優先、比對到就把那一段消耗掉。
   *
   * 實測於 RADWIMPS〈すずめ〉:照らすは熄み
   */
